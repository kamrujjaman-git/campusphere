...
export function OwnerCommunityList({ communities }: { communities: Community[] }) {
    ...
    return (
        <div className="space-y-6">
            ...
            {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><form onSubmit={save} className="w-full max-w-md space-y-4 rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-semibold">Edit Community</h2><input name="name" required defaultValue={editing.name} className="w-full rounded-lg border border-border bg-secondary px-3 py-2" placeholder="Name" /><input name="domain" required defaultValue={editing.domain} className="w-full rounded-lg border border-border bg-secondary px-3 py-2" placeholder="Domain" /><input name="key" required defaultValue={editing.key} className="w-full rounded-lg border border-border bg-secondary px-3 py-2" placeholder="Key" /><input name="admin_email" type="email" required defaultValue={editing.assignedAdminEmail} className="w-full rounded-lg border border-border bg-secondary px-3 py-2" placeholder="Super admin email" /><input name="logo" type="file" accept="image/jpeg,image/png,image/webp" className="w-full rounded-lg border border-border bg-secondary px-3 py-2 text-sm" /><div className="flex gap-2"><input type="hidden" name="status" value={editing.status} /><button type="button" onClick={() => setEditing({ ...editing, status: editing.status === "active" ? "suspended" : "active" })} className="rounded-lg border border-border px-3 py-2 text-sm">Status: {editing.status}</button><button type="submit" disabled={isPending} className="rounded-lg bg-primary px-4 py-2 text-sm text-primary-foreground">Save</button><button type="button" onClick={() => setEditing(null)} className="rounded-lg border border-border px-4 py-2 text-sm">Cancel</button></div></form></div>}
        </div>
    );
}
